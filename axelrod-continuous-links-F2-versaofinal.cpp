...
int main(int ac, char **av)
{
  ...
  individual.sigma_trait_mut = atof (av[++j]);
  config = atoi (av[++j]);
  ...
  while( (++conf)<=config )
      ...
      while( network.npilha>0 )
	{
      ...
      sprintf(arq2,"Output-PartialOutcomes-Axelrod-L%d-F%d-sigma%g-d%g.dat",network.L,individual.F,individual.sigma_trait_mut,individual.d);
      ptt2 = fopen(arq2,"a");
  ...
  sprintf(arq2,"Output-Optimized-Axelrod-L%d-F%d-sigma%g.dat",network.L,individual.F,individual.sigma_trait_mut);
  ptt2 = fopen(arq2,"a");
...
void dynamics(individuo *individual, rede *network)
{
  ...
  mut_effect = individual->state[network->pilha[ind_pilha][ind_neigh]][ind_seq] + gsl_ran_gaussian(gerador, individual->sigma_trait_mut);
  
  while( (mut_effect<0) || (mut_effect>1) )
    mut_effect = individual->state[network->pilha[ind_pilha][ind_neigh]][ind_seq] + gsl_ran_gaussian(gerador, individual->sigma_trait_mut);
